...
function buildResult100(){
  ...
  add('3유형 실행결과 맞추기 11~30 조건','전화번호 없는 고객 수','SELECT COUNT(*) FROM Customer WHERE phone IS NULL;',1,'NULL 조건');
  ...
  // 31~100: 61~100은 반복 없는 고난도 실행결과 문제
  const resultAdvanced = [
    ...
    ["주문 없는 고객 포함 고객별 총액", "SELECT Customer.name, COALESCE(SUM(Orders.saleprice),0) FROM Customer LEFT OUTER JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 39000, 김연아 15000, 장미란 31000, 추신수 33000, 박세리 0", "실제 테이블 기준 계산 결과입니다."],
    ["주문 없는 도서 포함 도서별 주문수와 매출", "SELECT Book.bookname, COUNT(Orders.orderid), COALESCE(SUM(Orders.saleprice),0) FROM Book LEFT OUTER JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.bookid, Book.bookname ORDER BY Book.bookid;", "축구의 역사 1 6000, 축구 아는 여자 1 12000, 축구의 이해 1 21000, 골프 바이블 0 0, 피겨 교본 1 8000, 역도 단계별기술 1 6000, 야구의 추억 1 20000, 야구를 부탁해 2 26000, 올림픽 이야기 0 0, Olympic Champions 2 19000", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 매출이 전체 평균 주문금액 초과", "SELECT Book.publisher, SUM(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher HAVING SUM(Orders.saleprice) > (SELECT AVG(saleprice) FROM Orders) ORDER BY SUM(Orders.saleprice) DESC;", "이상미디어 46000, 대한미디어 21000, 굿스포츠 20000, Pearson 19000, 나무수 12000", "실제 테이블 기준 계산 결과입니다."],
    ["고객 평균 주문금액이 전체 평균 이상", "SELECT Customer.name, AVG(Orders.saleprice) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name HAVING AVG(Orders.saleprice) >= (SELECT AVG(saleprice) FROM Orders) ORDER BY Customer.custid;", "박지성 13000, 추신수 16500", "실제 테이블 기준 계산 결과입니다."],
    ["주문별 할인액 1000 이상 상세", "SELECT Customer.name, Book.bookname, Book.price - Orders.saleprice FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid WHERE Book.price - Orders.saleprice >= 1000 ORDER BY Orders.orderid;", "박지성 축구의 역사 1000, 박지성 축구의 이해 1000, 박지성 축구 아는 여자 1000, 장미란 Olympic Champions 1000, 김연아 Olympic Champions 6000", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 할인액 합계 1000 이상", "SELECT Customer.name, SUM(Book.price - Orders.saleprice) FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid GROUP BY Customer.custid, Customer.name HAVING SUM(Book.price - Orders.saleprice) >= 1000 ORDER BY Customer.custid;", "박지성 3000, 김연아 6000, 장미란 1000", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 할인액 합계", "SELECT Book.publisher, SUM(Book.price - Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher ORDER BY SUM(Book.price - Orders.saleprice) DESC;", "Pearson 7000, 대한미디어 1000, 나무수 1000, 굿스포츠 1000, 이상미디어 0", "실제 테이블 기준 계산 결과입니다."],
    ["주문된 도서 중 정가 평균 이상 도서", "SELECT DISTINCT Book.bookname FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Book.price >= (SELECT AVG(price) FROM Book) ORDER BY Book.price;", "야구의 추억, 축구의 이해", "실제 테이블 기준 계산 결과입니다."],
    ["주문 안 된 도서 중 최고가", "SELECT bookname FROM Book WHERE NOT EXISTS (SELECT * FROM Orders WHERE Orders.bookid=Book.bookid) ORDER BY price DESC LIMIT 1;", "골프 바이블", "실제 테이블 기준 계산 결과입니다."],
    ["최근 주문일 고객과 도서", "SELECT Customer.name, Book.bookname FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid WHERE Orders.orderdate=(SELECT MAX(orderdate) FROM Orders);", "장미란 야구를 부탁해", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 마지막 주문일과 주문수", "SELECT Customer.name, MAX(Orders.orderdate), COUNT(*) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 2020-07-07 3, 김연아 2020-07-09 2, 장미란 2020-07-10 3, 추신수 2020-07-07 2", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 첫 주문일과 총액", "SELECT Customer.name, MIN(Orders.orderdate), SUM(Orders.saleprice) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 2020-07-01 39000, 김연아 2020-07-03 15000, 장미란 2020-07-04 31000, 추신수 2020-07-05 33000", "실제 테이블 기준 계산 결과입니다."],
    ["도서별 주문 고객 종류 2명 이상", "SELECT Book.bookname, COUNT(DISTINCT Orders.custid) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.bookid, Book.bookname HAVING COUNT(DISTINCT Orders.custid) >= 2 ORDER BY Book.bookid;", "야구를 부탁해 2, Olympic Champions 2", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 주문수 2건 이상", "SELECT Book.publisher, COUNT(Orders.orderid) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher HAVING COUNT(Orders.orderid) >= 2 ORDER BY MIN(Book.bookid);", "굿스포츠 3, 이상미디어 3, Pearson 2", "실제 테이블 기준 계산 결과입니다."],
    ["대한민국 고객의 주문 총액", "SELECT SUM(Orders.saleprice) FROM Customer JOIN Orders ON Customer.custid=Orders.custid WHERE Customer.address LIKE '대한민국%';", "46000", "실제 테이블 기준 계산 결과입니다."],
    ["주소 국가별 주문 수", "SELECT CASE WHEN Customer.address LIKE '대한민국%' THEN '대한민국' WHEN Customer.address LIKE '미국%' THEN '미국' ELSE '기타' END, COUNT(Orders.orderid) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY CASE WHEN Customer.address LIKE '대한민국%' THEN '대한민국' WHEN Customer.address LIKE '미국%' THEN '미국' ELSE '기타' END ORDER BY COUNT(Orders.orderid) DESC;", "대한민국 5, 기타 3, 미국 2", "실제 테이블 기준 계산 결과입니다."],
    ["굿스포츠 또는 이상미디어 주문 고객", "SELECT DISTINCT Customer.name FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid WHERE Book.publisher IN ('굿스포츠','이상미디어') ORDER BY Customer.custid;", "박지성, 김연아, 장미란, 추신수", "실제 테이블 기준 계산 결과입니다."],
    ["주문 금액이 고객 평균 이상인 주문번호", "SELECT o1.orderid FROM Orders o1 WHERE o1.saleprice >= (SELECT AVG(o2.saleprice) FROM Orders o2 WHERE o2.custid=o1.custid) ORDER BY o1.orderid;", "2, 3, 5, 8, 10", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 자기 평균 이상 주문 수", "SELECT Customer.name, COUNT(*) FROM Customer JOIN Orders o1 ON Customer.custid=o1.custid WHERE o1.saleprice >= (SELECT AVG(o2.saleprice) FROM Orders o2 WHERE o2.custid=o1.custid) GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 1, 김연아 1, 장미란 2, 추신수 1", "실제 테이블 기준 계산 결과입니다."],
    ["출판사 평균 정가 이상으로 팔린 주문수", "SELECT Book.publisher, COUNT(*) FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Book.price >= (SELECT AVG(b2.price) FROM Book b2 WHERE b2.publisher=Book.publisher) GROUP BY Book.publisher ORDER BY MIN(Book.bookid);", "굿스포츠 2, 나무수 1, 이상미디어 1, Pearson 2", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 주문 도서 평균 정가", "SELECT Customer.name, AVG(Book.price) FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 14000, 김연아 10500, 장미란 10666.666666666666, 추신수 16500", "실제 테이블 기준 계산 결과입니다."],
    ["도서명 축구 포함 주문 총액", "SELECT SUM(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Book.bookname LIKE '%축구%';", "39000", "실제 테이블 기준 계산 결과입니다."],
    ["도서명 야구 포함 주문 총액", "SELECT SUM(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Book.bookname LIKE '%야구%';", "46000", "실제 테이블 기준 계산 결과입니다."],
    ["주문 없는 고객 포함 평균 주문금액", "SELECT Customer.name, COALESCE(AVG(Orders.saleprice),0) FROM Customer LEFT OUTER JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 13000, 김연아 7500, 장미란 10333.333333333334, 추신수 16500, 박세리 0", "실제 테이블 기준 계산 결과입니다."],
    ["주문 없는 도서 포함 평균 판매가", "SELECT Book.bookname, COALESCE(AVG(Orders.saleprice),0) FROM Book LEFT OUTER JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.bookid, Book.bookname ORDER BY Book.bookid;", "축구의 역사 6000, 축구 아는 여자 12000, 축구의 이해 21000, 골프 바이블 0, 피겨 교본 8000, 역도 단계별기술 6000, 야구의 추억 20000, 야구를 부탁해 13000, 올림픽 이야기 0, Olympic Champions 9500", "실제 테이블 기준 계산 결과입니다."],
    ["판매가가 정가보다 낮은 출판사별 주문수", "SELECT Book.publisher, COUNT(*) FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Orders.saleprice < Book.price GROUP BY Book.publisher ORDER BY MIN(Book.bookid);", "굿스포츠 1, 나무수 1, 대한미디어 1, Pearson 2", "실제 테이블 기준 계산 결과입니다."],
    ["할인 없는 주문 고객명", "SELECT Customer.name FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid WHERE Orders.saleprice = Book.price ORDER BY Orders.orderid;", "김연아, 장미란, 추신수, 추신수, 장미란", "실제 테이블 기준 계산 결과입니다."],
    ["주문 도서 중 최고 정가 도서명", "SELECT DISTINCT Book.bookname FROM Book JOIN Orders ON Book.bookid=Orders.bookid WHERE Book.price = (SELECT MAX(Book.price) FROM Book JOIN Orders ON Book.bookid=Orders.bookid);", "축구의 이해", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 주문수와 도서종류수", "SELECT Customer.name, COUNT(*), COUNT(DISTINCT Orders.bookid) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY Customer.custid;", "박지성 3 3, 김연아 2 2, 장미란 3 3, 추신수 2 2", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 고객종류수와 매출", "SELECT Book.publisher, COUNT(DISTINCT Orders.custid), SUM(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher ORDER BY SUM(Orders.saleprice) DESC;", "이상미디어 2 46000, 대한미디어 1 21000, 굿스포츠 3 20000, Pearson 2 19000, 나무수 1 12000", "실제 테이블 기준 계산 결과입니다."],
    ["매출 2위 고객명", "SELECT Customer.name FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY SUM(Orders.saleprice) DESC LIMIT 1 OFFSET 1;", "추신수", "실제 테이블 기준 계산 결과입니다."],
    ["주문수 2위 고객명", "SELECT Customer.name FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name ORDER BY COUNT(*) DESC, SUM(Orders.saleprice) DESC LIMIT 1 OFFSET 1;", "장미란", "실제 테이블 기준 계산 결과입니다."],
    ["판매가 상위 3건 도서명", "SELECT Book.bookname FROM Book JOIN Orders ON Book.bookid=Orders.bookid ORDER BY Orders.saleprice DESC, Orders.orderid ASC LIMIT 3;", "축구의 이해, 야구의 추억, 야구를 부탁해", "실제 테이블 기준 계산 결과입니다."],
    ["날짜별 주문수와 총액 2건 이상", "SELECT orderdate, COUNT(*), SUM(saleprice) FROM Orders GROUP BY orderdate HAVING COUNT(*) >= 2 ORDER BY orderdate;", "2020-07-03 2 29000, 2020-07-07 2 25000", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 총액이 전체 고객 평균 총액 이상", "SELECT Customer.name, SUM(Orders.saleprice) FROM Customer JOIN Orders ON Customer.custid=Orders.custid GROUP BY Customer.custid, Customer.name HAVING SUM(Orders.saleprice) >= (SELECT AVG(t.total) FROM (SELECT SUM(saleprice) AS total FROM Orders GROUP BY custid) t) ORDER BY Customer.custid;", "박지성 39000, 장미란 31000, 추신수 33000", "실제 테이블 기준 계산 결과입니다."],
    ["도서별 매출이 주문 도서 평균 매출 이상", "SELECT Book.bookname, SUM(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.bookid, Book.bookname HAVING SUM(Orders.saleprice) >= (SELECT AVG(t.total) FROM (SELECT SUM(saleprice) AS total FROM Orders GROUP BY bookid) t) ORDER BY Book.bookid;", "축구의 이해 21000, 야구의 추억 20000, 야구를 부탁해 26000, Olympic Champions 19000", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 평균 판매가", "SELECT Book.publisher, AVG(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher ORDER BY MIN(Book.bookid);", "굿스포츠 6666.666666666667, 나무수 12000, 대한미디어 21000, 이상미디어 15333.333333333334, Pearson 9500", "실제 테이블 기준 계산 결과입니다."],
    ["고객별 최고 판매가 도서명", "SELECT Customer.name, Book.bookname FROM Customer JOIN Orders ON Customer.custid=Orders.custid JOIN Book ON Orders.bookid=Book.bookid WHERE Orders.saleprice = (SELECT MAX(o2.saleprice) FROM Orders o2 WHERE o2.custid=Customer.custid) ORDER BY Customer.custid;", "박지성 축구의 이해, 김연아 피겨 교본, 장미란 야구를 부탁해, 추신수 야구의 추억", "실제 테이블 기준 계산 결과입니다."],
    ["출판사별 최저 판매가 주문", "SELECT Book.publisher, MIN(Orders.saleprice) FROM Book JOIN Orders ON Book.bookid=Orders.bookid GROUP BY Book.publisher ORDER BY MIN(Book.bookid);", "굿스포츠 6000, 나무수 12000, 대한미디어 21000, 이상미디어 13000, Pearson 7000", "실제 테이블 기준 계산 결과입니다."],
    ["주문 없는 고객과 주문 없는 도서 수 합계", "SELECT (SELECT COUNT(*) FROM Customer WHERE NOT EXISTS (SELECT * FROM Orders WHERE Orders.custid=Customer.custid)) + (SELECT COUNT(*) FROM Book WHERE NOT EXISTS (SELECT * FROM Orders WHERE Orders.bookid=Book.bookid));", "3", "실제 테이블 기준 계산 결과입니다."],
  ];
